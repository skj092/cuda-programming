...
void mykernel(float *h_m, float *h_n, float *h_o, int N){
    ...
    ms[threadIdx.y][threadIdx.x] = h_m[col * N + row];
    ns[threadIdx.x][threadIdx.y] = h_n[row * N + col];

    __syncthreads();

    float p = 0;
    for (int i = 0; i < TW; i++){
        p += ms[threadIdx.y][i] * ns[i][threadIdx.x];
    }
    h_o[row * N + col] = p;
    __syncthreads();
}
...
int main(){
    int N = 6;
    float *h_m, *h_n, *h_o; // host variable
    float *d_m, *d_n, *d_o; // device variable
